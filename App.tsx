...
function getChannelList(callback: any) {
    ...
    const order: GroupChannelOrder | any = sb.GroupChannelCollection.GroupChannelOrder.LATEST_LAST_MESSAGE;
...
function getMessagesFromChannel(groupChannel: any, callback: any) {
    ...
    var messageCollectionHandler = {
        onMessagesAdded: (context: MessageContext, channel: BaseChannel, messages: BaseMessageInstance[]) => {
            // Add the messages to your data source.
            console.log(`A new message was added. If you see the pendingStatus attribute for this message is: "pending"`);
            console.log(messages);
        },
        onMessagesUpdated: (context: MessageContext, channel: BaseChannel, messages: BaseMessageInstance[]) => {
            // Update the messages in your data source.
            console.log('Message was updated');
            console.log(messages);
        },
        onMessagesDeleted: (context: MessageContext, channel: BaseChannel, messages: BaseMessageInstance[]) => {
        ...
        onHugeGapDetected: () => {
            ...
            console.log('Sendbird needs newer data  not continue with any cache action.');
...
function sendMessage(groupChannel: any, messageText: string, callback: any) {
    ...
    groupChannel.sendUserMessage(params, (userMessage: UserMessage | FileMessage | AdminMessage, error: SendBirdError) => {
        if (error) {
            ...
            <FlatList
                ...
                renderItem={(itemSelected: any) =>
                    <TouchableOpacity onPress={() => {
                        ...
                        getMessagesFromChannel(itemSelected.item, (isFromCache: boolean, messages: any) => {
